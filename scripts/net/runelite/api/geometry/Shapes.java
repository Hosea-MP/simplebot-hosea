...
implements Shape {
    ...
    public PathIterator getPathIterator(AffineTransform at2) {
        return new ShapeIterator(this.shapes.stream().map(s -> s.getPathIterator(at2)).iterator());
    }

    @Override
    public PathIterator getPathIterator(AffineTransform at2, double flatness) {
        return new ShapeIterator(this.shapes.stream().map(s -> s.getPathIterator(at2, flatness)).iterator());
    }
