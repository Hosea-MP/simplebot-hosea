...
public class Geometry {
    ...
    public static List<Point2D.Float> intersectionPoints(Shape shape, float x1, float y1, float x2, float y2) {
        ...
        PathIterator it2 = shape.getPathIterator(new AffineTransform());
        float[] coords = new float[2];
        float[] prevCoords = new float[2];
        float[] start = new float[2];
        while (!it2.isDone()) {
            Point2D.Float intersection;
            int type = it2.currentSegment(coords);
            if (type == 0) {
                start[0] = coords[0];
                start[1] = coords[1];
    ...
    public static GeneralPath transformPath(PathIterator it2, Consumer<float[]> method) {
    ...
    private static void appendSegmentLines(GeneralPath path, float segmentLength, float x1, float y1, float x2, float y2) {
        ...
        float dx2 = (float)Math.cos(angle) * segmentLength;
        float dy2 = (float)Math.sin(angle) * segmentLength;
        float length = (float)Math.hypot(x2 - x1, y2 - y1);
        int steps = (int)(((double)length - 1.0E-4) / (double)segmentLength);
        for (int i = 0; i < steps; ++i) {
            path.lineTo(x += dx2, y += dy2);
        }
    }

    public static GeneralPath splitIntoSegments(PathIterator it2, float segmentLength) {
    ...
    public static GeneralPath filterPath(PathIterator it2, BiPredicate<float[], float[]> method) {
        GeneralPath newPath = new GeneralPath();
        float[] prevCoords = new float[2];
        float[] coords = new float[2];
        float[] start = new float[2];
    ...
    public static GeneralPath clipPath(PathIterator it2, Shape shape) {
        GeneralPath newPath = new GeneralPath();
        float[] prevCoords = new float[2];
        float[] coords = new float[2];
        float[] start = new float[2];
