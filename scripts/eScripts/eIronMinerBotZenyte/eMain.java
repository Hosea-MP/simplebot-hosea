    ...
    public void miningTask(SimpleObjects ironObjects) {
        ...
        for (WorldPoint position : MINING_POSITION_TO_ORE_MAP.keySet()) {
            ...
            if (!miningSpotIsOccupied(position) && !currentLocation.equals(position)) {
                eScripts.BotUtils.eActions.status = "Getting to mining spot";
        ...
        if (!movedToPosition) {
            eScripts.BotUtils.eActions.status = "Waiting for a mining position to free up";
        }
    }


    private void startMining(SimpleObject objectName) {
        if (!ctx.players.getLocal().isAnimating() && !ctx.pathing.inMotion()) eScripts.BotUtils.eActions.interactWith(objectName, "Mine");
    }
