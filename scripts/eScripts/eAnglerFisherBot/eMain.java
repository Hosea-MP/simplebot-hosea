...
public class eMain extends TaskScript implements LoopingScript {
    ...
    public void onProcess() {
        ...
        if (!myInventory.inventoryFull() && !eScripts.BotUtils.eBanking.bankIsOpen()) {
            ...
            if (!localPlayer.isAnimating() && (System.currentTimeMillis() > (lastAnimation + eScripts.BotUtils.eActions.getRandomInt(1000, 5000)))) {
                fishingTask();
                eScripts.BotUtils.eActions.status = "Fishing...";
        ...
        if (eScripts.BotUtils.eActions.hasItemsInInventory(null, ItemID.ANGLER_HAT, ItemID.ANGLER_TOP, ItemID.ANGLER_WADERS, ItemID.ANGLER_BOOTS)) {
            eScripts.BotUtils.eActions.updateStatus("Wearing Anglers piece");
            eScripts.BotUtils.eActions.handleInventoryItem("Wear", ItemID.ANGLER_HAT, ItemID.ANGLER_TOP, ItemID.ANGLER_WADERS, ItemID.ANGLER_BOOTS);
        }

        eScripts.BotUtils.eActions.handleGroundItem("Take", ItemID.RAW_ANGLERFISH, ItemID.ANTIQUE_LAMP_7498);

        eScripts.BotUtils.eActions.handleRunning();

        if (localPlayer.isAnimating()) {
            if (eScripts.BotUtils.eActions.specialAttackTool) {
                eScripts.BotUtils.eActions.specialAttack(eScripts.BotUtils.eData.Fishing.SPECIAL_ATTACK_TOOL);
            }
        }
    }
    ...
    private void fishingTask() {
        ...
        eScripts.BotUtils.eActions.walkPath(FISHING_AREA, WALKING_TO_ANGLER_SPOT, false);
        ...
        eScripts.BotUtils.eActions.interactWith(fishingSpot, "Bait");
        ctx.onCondition(localPlayer::isAnimating, 250, 10);
    }

    private void bankingTask() {
        eScripts.BotUtils.eActions.walkPath(BANK_AREA, WALKING_TO_ANGLER_SPOT, true);
        eScripts.BotUtils.eBanking.bankTask(true, 8, 1, false, -1, -1, ItemID.FISHING_ROD, ItemID.SANDWORMS);
    }
