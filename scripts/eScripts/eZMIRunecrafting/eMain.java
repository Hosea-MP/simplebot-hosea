...
import static eScripts.BotUtils.eActions.random;
...
public class eMain extends TaskScript implements LoopingScript {
    ...
    private void performBankingTasks() {
        ...
        if (!essenceInInv || !ctx.inventory.inventoryFull()) {
            ...
            eScripts.BotUtils.eActions.interactWith(banker, "Last Preset");
            return;
        }
    ...
    private void runecraftAtAltar() {
        if (!ctx.pathing.inArea(OURANIA_RC_ALTAR_AREA)) {
            ...
            if (random.nextDouble() < 0.75) {
                eScripts.BotUtils.eActions.walkPath(OURANIA_RC_ALTAR_AREA, WALKING_TO_ALTAR2, false);
            } else {
                eScripts.BotUtils.eActions.walkPath(OURANIA_RC_ALTAR_AREA, WALKING_TO_ALTAR, false);
            }
...
/*    private void useItemOnItemFletch() {
        ...
        eScripts.BotUtils.eActions.status = "Fletching";
        featherInv.click(0);
        itemInv.click(0);
    }*/
    ...
    private void useItemOnItemFletch() {
        ...
        if (!eBanking.bankIsOpen()) {
            eScripts.BotUtils.eActions.status = "Fletching " + itemInv.getName().toLowerCase();
            featherInv.click(0);
            itemInv.click(0);
            ctx.sleep(50);
        }
    ...
    private void outOfTeleportRunes() {
        ...
        if (!ctx.pathing.inArea(OURANIA_BANKING_AREA)) {
            ...
            setPrayers(prayerIsOn);
            eScripts.BotUtils.eActions.walkPath(OURANIA_BANKING_AREA, WALKING_TO_ALTAR, true);
        } else {
            setBooleans(false);
        }
    }
