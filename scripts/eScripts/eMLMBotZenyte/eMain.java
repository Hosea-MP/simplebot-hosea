...
public class eMain extends TaskScript implements LoopingScript {
    ...
    private void initializeMethods() {
        ...
        eScripts.BotUtils.eData dataUtils = new eScripts.BotUtils.eData(ctx);
...
/*        if (redwoodMode) {
    ...
    private void handleGroundItem() {
        ...
        if (itemToPickup != null && itemToPickup.validateInteractable()) {
            updateStatus(eScripts.BotUtils.eActions.getCurrentTimeFormatted() + " Found " + itemToPickup.getName());
    ...
    private void specialAttack(SimplePlayer localPlayer) {
        ...
        if (!hasSpecialAttackTool) {
            updateStatus(eScripts.BotUtils.eActions.getCurrentTimeFormatted() + " Special attack tool: NOT FOUND");
            updateStatus(eScripts.BotUtils.eActions.getCurrentTimeFormatted() + " Special attack: Deactivated");
            specialAttackTool = false;
            return;
        }

        if (localPlayer.isAnimating() && ctx.combat.toggleSpecialAttack(true)) {
            updateStatus(eScripts.BotUtils.eActions.getCurrentTimeFormatted() + " Used special attack");
    ...
    private void useBankObject(SimpleObject objectName) {
        ...
        if (!isBankReachabe) {
            ...
            if (rockFalls != null) {
                eScripts.BotUtils.eActions.interactWith(rockFalls, "Mine");
                return;
            }
        }
