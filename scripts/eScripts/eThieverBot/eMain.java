...
public class eMain extends TaskScript implements LoopingScript {
       ...
       if (inLletya) {
           if (playerHealth > 9) {
               if (ctx.players.getLocal().getGraphic() != -1) {
                   eScripts.BotUtils.eActions.status = "Stunned";
                   return;
               }
        ...
        if (inEdge) {
            ...
            if (playerHealth < 10) {
                eScripts.BotUtils.eActions.status = "Restoring hitpoints";
                ...
                if (!eScripts.BotUtils.eActions.hasItemsInInventory(eActions.StackableType.NON_STACKABLE, foodToEat)) {
                    eScripts.BotUtils.eActions.status = "Banking";
        ...
        if (eActions.hasItemsInInventory(eActions.StackableType.NON_STACKABLE, ItemID.ROGUE_MASK, ItemID.ROGUE_TOP, ItemID.ROGUE_TROUSERS, ItemID.ROGUE_BOOTS, ItemID.ROGUE_GLOVES)) {
            eScripts.BotUtils.eActions.updateStatus("Wearing Rogue piece");
    ...
    private void handlePouch(int amount) {
        if (eScripts.BotUtils.eActions.getNotedItemCount("Coin pouch") >= amount) {
    ...
    private void openDoor(String name) {
        ...
        if (theDoor != null) {
            eScripts.BotUtils.eActions.status = "Opening door to get to " + name;
            eScripts.BotUtils.eActions.interactWith(theDoor, "Open");
    ...
    private void interactWithNpc(SimpleNpc nearestNpc) {
        ...
        eScripts.BotUtils.eActions.status = "Thieving " + nearestNpc.getName();
        eScripts.BotUtils.eActions.interactWith(nearestNpc, "Pickpocket");
    }
