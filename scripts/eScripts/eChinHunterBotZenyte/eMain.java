...
public class eMain extends TaskScript implements LoopingScript {
    ...
    public void onExecute() {
        ...
        // Starting message
        this.ctx.log("--------------- " + eScripts.BotUtils.eActions.getCurrentTimeFormatted() + " ---------------");
        this.ctx.log("-------------------------------------");
    ...
    public void onProcess() {
        ...
        if (!trapsPickup) {
            ...
            if (floorTrap != null && floorTrap.validateInteractable() && !localPlayer.isAnimating()) {
                eScripts.BotUtils.eActions.status = "Laying trap";
                ...
                eScripts.BotUtils.eActions.interactWith(floorTrap, "Lay");
            ...
            if (placedTraps() == trapAmount() && trapTile == null && floorTrap == null) {
                eScripts.BotUtils.eActions.status = "Waiting for action";
                ...
                if (trap != null && trap.validateInteractable() && !localPlayer.isAnimating()) {
                    eScripts.BotUtils.eActions.status = "Resetting traps";
                    int chinsCached = getChinCount();
                    eScripts.BotUtils.eActions.interactWith(trap, "Reset");
                    ctx.sleep(3000);
                ...
                eScripts.BotUtils.eActions.status = "Checking traps in inventory";
                ...
                if (!playerLocation.equals(trapTile)) {
                    eScripts.BotUtils.eActions.status = "Walking to next spot";
                    stepOnTile(trapTile.getX(), trapTile.getY(), playerLocation);
                    ctx.onCondition(() -> true, 200, 8);
                }

                if (playerLocation.equals(trapTile)) {
                    eScripts.BotUtils.eActions.status = "Setting up trap";
            ...
            if (!groundItems.isEmpty()) {
                if (!ctx.inventory.inventoryFull()) {
                    ...
                    eScripts.BotUtils.eActions.status = "Picking up trap(s)";
                    ...
                    if (trapsGround != null && trapsGround.validateInteractable() && !localPlayer.isAnimating()) {
                        ...
                        eScripts.BotUtils.eActions.interactWith(trapsGround, "Take");
                        ctx.onCondition(() -> ctx.inventory.populate().filter("Box trap").population() > trapsInv);
                    }
                }
            } else {
                eScripts.BotUtils.eActions.status = "No traps to pickup";
                trapsPickup = false;
            }
        }
    }
...
/*        String eventToString = getEvent.toString().replaceAll("<[^>]+>", "").trim();
        ...
        if (getType == ChatMessageType.GAMEMESSAGE) {
            if (gameMessage.contains("You cannot lay a trap here")) {
                eScripts.BotUtils.eActions.status = "Can't lay here!";
                trapsPickup = true;
            }
            ...
            for (WorldPoint trapTile : locs) {
                if (trapTile != null) {
                    if (trapWithAction(EntityType.SimpleGroundItem, trapTile, "Take")) {
                        eScripts.BotUtils.eActions.drawTileMatrix(ctx, (Graphics2D) g, trapTile, Color.RED);
                    } else if (trapWithAction(EntityType.SimpleObjects,trapTile, "Reset")) {
                        eScripts.BotUtils.eActions.drawTileMatrix(ctx, (Graphics2D) g, trapTile, Color.GREEN);
                        ...
                        eScripts.BotUtils.eActions.drawTileMatrix(ctx, (Graphics2D) g, trapTile, Color.YELLOW);
                    } else {
                        eScripts.BotUtils.eActions.drawTileMatrix(ctx, (Graphics2D) g, trapTile, Color.DARK_GRAY);
                    }
                }
            }
        }
