package eScripts.eWoodcuttingBotZenyte;

import eScripts.BotUtils.eActions;
import eScripts.BotUtils.eBanking;
import eScripts.BotUtils.eData;
import eScripts.BotUtils.eImpCatcher;
import eScripts.Trivia.eTriviaInfo;
import eScripts.eApiAccess.eAutoResponderGui;
import eScripts.eApiAccess.eAutoResponser;
...
import static eScripts.eApiAccess.eAutoResponser.*;
...
public class eMain extends TaskScript implements LoopingScript {
    ...
    public void onExecute() {
        ...
        // Other vars
        ctx.log("--------------- " + eScripts.BotUtils.eActions.getCurrentTimeFormatted() + " ---------------");
        ctx.log("-------------------------------------");
        ctx.log("            " + eBotName + "         ");
        ctx.log("-------------------------------------");

        // Vars
        eScripts.BotUtils.eActions.updateStatus("Setting up bot");
    ...
    public void onProcess() {
        ...
        if (!botStarted || treeName == null) {
            eScripts.BotUtils.eActions.status = "Press start & examine any tree";
            return;
        }

        if (redwoodMode) {
            if (!myInventory.inventoryFull() && !eScripts.BotUtils.eBanking.bankIsOpen()) {
                ...
                if (localPlayer.getLocation().getPlane() != 1 && ctx.players.getLocal().getLocation().getRegionID() == 6198) {
                    ...
                    if (!localPlayer.isAnimating() && !pathing.inMotion() && (System.currentTimeMillis() > (lastAnimation + eScripts.BotUtils.eActions.getRandomInt(1200, 3200)))) {
                ...
                if (localPlayer.getLocation().getPlane() == 1 && ctx.players.getLocal().getLocation().getRegionID() == 6198) {
                    ...
                    eScripts.BotUtils.eBanking.bankTask(true, 8, 1, false, (String) null, -1, eData.Woodcutting.WOODCUTTING_AXE);
                }
            }
        ...
        if (!redwoodMode) {
            if (!myInventory.inventoryFull() && !eScripts.BotUtils.eBanking.bankIsOpen()) {
                ...
                if (!localPlayer.isAnimating() && !pathing.inMotion() && (System.currentTimeMillis() > (lastAnimation + eScripts.BotUtils.eActions.getRandomInt(1000, 5000)))) {
                    cuttingTrees(localPlayer, treeName);
                } else if (localPlayer.isAnimating()) {
                    lastAnimation = System.currentTimeMillis();
                }
        ...
        eScripts.BotUtils.eActions.handleRunning();
        ...
        if (!myInventory.populate().filter(ItemID.LUMBERJACK_HAT, ItemID.LUMBERJACK_TOP, ItemID.LUMBERJACK_LEGS, ItemID.LUMBERJACK_BOOTS).isEmpty()) {
            eScripts.BotUtils.eActions.updateStatus("Wearing Lumberjack piece");
        ...
        if (!ctx.inventory.inventoryFull()) {
            eScripts.BotUtils.eActions.handleGroundItem("Take", BIRD_NEST);
        }

        if (localPlayer.isAnimating()) {
            if (eScripts.BotUtils.eActions.specialAttackTool) {
                eScripts.BotUtils.eActions.specialAttack(SPECIAL_ATTACK_TOOL);
            }
        }
    }
    ...
    private void cuttingTrees(SimplePlayer localPlayer, String nameOfTree) {
        ...
        eScripts.BotUtils.eActions.updateStatus("Looking for " + nameOfTree + " trees...");
        ...
        while (!treesNearby.isEmpty()) {
            ...
            if (treesNearby.size() >= 2 && isOtherPlayerChopping) {
                eScripts.BotUtils.eActions.updateStatus("Another player is chopping the nearest " + nameOfTree + ".");
                eScripts.BotUtils.eActions.updateStatus("Looking for another tree...");
                ...
                if (nearestTree.validateInteractable()) {
                    ...
                    if (reachable) {
                        ...
                        eScripts.BotUtils.eActions.updateStatus(nameOfTree + " found " + (treeLocation.distanceTo(ctx.players.getLocal().getLocation()) - 1) + eScripts.BotUtils.eActions.pluralize(distance, " tile", " tiles") + " away");
                        if (redwoodMode) {
                            ctx.viewport.pitch(true);
                        }
                        eScripts.BotUtils.eActions.interactWith(nearestTree, treeAction);
                        eScripts.BotUtils.eActions.status = "Chopping " + nameOfTree;
                        eScripts.BotUtils.eActions.updateStatus(eScripts.BotUtils.eActions.status);
                        ctx.onCondition(localPlayer::isAnimating, 250, 10);
                        return;
                    } else {
                        eScripts.BotUtils.eActions.updateStatus("Next " + nameOfTree + " is not reachable.");
                        treesNearby = (SimpleObjects) treesNearby.filter(otherTree -> !otherTree.equals(nearestTree));
                    }
                } else {
                    eScripts.BotUtils.eActions.updateStatus("No " + nameOfTree + " found in the vicinity.");
                    return;
                }
            }
        }
        eScripts.BotUtils.eActions.updateStatus("No suitable " + nameOfTree + " trees found nearby.");
    }
    ...
    private void handleGroundItem() {
        ...
        if (itemToPickup != null && itemToPickup.validateInteractable()) {
            eScripts.BotUtils.eActions.updateStatus(eScripts.BotUtils.eActions.getCurrentTimeFormatted() + " Found " + itemToPickup.getName());
    ...
    private void handleRopeLadder(SimplePlayer localPlayer, String action, int expectedPlane) {
        ...
        eScripts.BotUtils.eActions.interactWith(ropeLadder, action);
        ...
        if (getType == ChatMessageType.OBJECT_EXAMINE) {
            ...
            for (eDataTrees treeData : eDataTrees.allTreesData) {
                if (examineMessage.contains(treeData.examineResult)) {
                    ...
                    eScripts.BotUtils.eActions.updateStatus("New tree selected: " + treeName);
                    break;
                }
            }
        }
    ...
    public void paint(Graphics g) {
        ...
        if (!hidePaint) {
            ...
            g.drawString("Status: " + eScripts.BotUtils.eActions.status, 15, 225);
