...
public class eMain extends TaskScript implements LoopingScript {
    ...
    public void onExecute() {
        ...
        eScripts.BotUtils.eActions.zoomOutViewport();
    }
    ...
    public void onProcess() {
        ...
        if (!eScripts.BotUtils.eActions.hasItemsInInventory(eActions.StackableType.BOTH, AMMO_MOULD)) eScripts.BotUtils.eBanking.bankTask(false, -1, -1, true, AMMO_MOULD, 1, -1);

        if (eScripts.BotUtils.eActions.hasItemsInInventory(eActions.StackableType.BOTH, STEEL_BAR)) {
            if (localPlayer.getAnimation() == -1 && (eScripts.BotUtils.eActions.getCurrentTimeMilli() > (lastAnimation + eScripts.BotUtils.eActions.getRandomInt(3000, 4000)))) {
                useFurnace();
            } else if (localPlayer.getAnimation() != -1) {
                lastAnimation = eScripts.BotUtils.eActions.getCurrentTimeMilli();
            }
        } else {
            eScripts.BotUtils.eBanking.bankTask(false, -1, 1, true, STEEL_BAR, 27, AMMO_MOULD);
        }
    }
...
/*    private void useFurnace() {
        ...
        if (eScripts.BotUtils.eBanking.bankIsOpen()) ctx.bank.closeBank();

        if (widget == null) {
            if (furnace != null && furnace.validateInteractable()) {
                eScripts.BotUtils.eActions.updateStatus("Clicking furnace");
                furnace.click("Smelt");
            ...
            if (!widget.isHidden()) {
                eScripts.BotUtils.eActions.updateStatus("Making cannonballs");
                widget.click(0);
    ...
    private void useFurnace() {
        ...
        if (furnace != null && furnace.validateInteractable()) {
            ...
            eScripts.BotUtils.eActions.updateStatus("Clicking " + furnace.getName().toLowerCase());
            WorldPoint playerLocation = ctx.players.getLocal().getLocation();
            WorldPoint furnaceLocation = furnace.getLocation();
        ...
        if (widgetVisible()) {
            int steelBarsCached = eScripts.BotUtils.eActions.getItemCountInventory(STEEL_BAR);
            eScripts.BotUtils.eActions.clickWidget(270, 14);
            ctx.onCondition(() -> steelBarsCached > eScripts.BotUtils.eActions.getItemCountInventory(STEEL_BAR), 50, 20);
        }
    }
