...
/*    // Define the data structure
    ...
    public void onProcess() {
        ...
        if (oresOnConveyor) {
            ...
            if (eScripts.BotUtils.eActions.hasItemsInInventory(null, BARS_LIST)) {
                oresOnConveyor = false;
                return;
            }

            if (localPlayer.getLocation().distanceTo(TAKE_BARS_FROM_DISPENSER_LOCATION) > 2) {
                eScripts.BotUtils.eActions.status = "Walking to bar dispenser";
            ...
            if (widgetVisible(270, 14)) {
                ...
                ctx.onCondition(() -> eScripts.BotUtils.eActions.hasItemsInInventory(null, BARS_LIST), 50, 20);
                return;
            }
        }
        ...
        if (!oresOnConveyor) {
            ...
            if (eScripts.BotUtils.eActions.hasItemsInInventory(null, ORES_LIST)) {
                count += countInv;
                countInv = 0;
                if (!pathing.inMotion()) pathing.step(NEAR_CONVAYOR_BELT_LOCATION);
                wearGloves(GOLDSMITH_GAUNTLETS);
                clickObject(ObjectID.CONVEYOR_BELT, "Put-ore-on", !eScripts.BotUtils.eActions.hasItemsInInventory(null, ORES_LIST));
                return;
            }

            if (eScripts.BotUtils.eActions.hasItemsInInventory(null, BARS_LIST)
                    || (!eScripts.BotUtils.eActions.hasItemsInInventory(null, BARS_LIST) && !eScripts.BotUtils.eActions.hasItemsInInventory(null, ORES_LIST))) {
                bankTask();
            }
        }
    }
    ...
    private void bankTask() {
        ...
        eScripts.BotUtils.eActions.status = "Banking";
        if (bankChest != null && bankChest.validateInteractable()) {
            eScripts.BotUtils.eActions.interactWith(bankChest, "Last Preset");
            ctx.onCondition(() -> !eScripts.BotUtils.eActions.hasItemsInInventory(null, ORES_LIST), 50, 20);
        }
    }
    ...
    private void handleEnergy() {
        ...
        if (pathing.energyLevel() < 20) {
            ...
            eScripts.BotUtils.eActions.status = ("Drinking " + potion.getName().toLowerCase());
    ...
    private void wearGloves(int glovesInt) {
        ...
        if (theGloves != null) {
            eScripts.BotUtils.eActions.status = "Wearing " + theGloves.getName().toLowerCase();
            if (theGloves.click(0)) {
                ctx.onCondition(() -> !eScripts.BotUtils.eActions.hasItemsInInventory(null, glovesInt), 50, 20);
            }
        }
    }
    ...
    private void clickObject(int objetId, String actionName, boolean condition) {
        ...
        if (objectToClick != null) {
            ...
            eScripts.BotUtils.eActions.status = "Clicking " + objectName.toLowerCase();
            eScripts.BotUtils.eActions.interactWith(objectToClick, actionName);
    ...
    public void onChatMessage(ChatMessage m) {
        ...
        if (getType == ChatMessageType.GAMEMESSAGE) {
            if (gameMessage.contains("your ore goes onto the conveyor belt")) {
                eScripts.BotUtils.eActions.status = "Ores are on conveyor belt!";
                oresOnConveyor = true;
            }
        }
    }
