...
import static eScripts.eApiAccess.eAutoResponser.randomSleeping;
...
/*    public static boolean hasItemsInInventory(StackableType type, int... itemIds) {
        ...
        if (!ctx.teleporter.open()) {
            ...
            if (zenytePortal != null) {
                if (ctx.players.getLocal().getLocation().distanceTo(NEAR_PORTAL_TILE) > 5) {
                    eScripts.BotUtils.eActions.status = "Running to portal";
                    ctx.pathing.step(NEAR_PORTAL_TILE);
                    ctx.onCondition(zenytePortal::visibleOnScreen, 250, 5);
                } else {
                    eScripts.BotUtils.eActions.status = "Clicking portal";
                    eScripts.BotUtils.eActions.interactWith(zenytePortal, "Teleport");
                    ctx.onCondition(() -> ctx.teleporter.open(), 250, 5);
                }
            }
        } else {
            eScripts.BotUtils.eActions.status = "Choosing destination...";
            ctx.teleporter.teleportStringPath(menuElement, locationName);
            ctx.game.tab(Game.Tab.INVENTORY);
    ...
    public static void teleportHomeSpellbook() {
        ...
        eScripts.BotUtils.eActions.status = "Teleporting to home";
        eScripts.BotUtils.eActions.openTab(Game.Tab.MAGIC);
        ...
        if (widgetNumber != -1) {
            eScripts.BotUtils.eActions.clickWidget(218, widgetNumber);
        }
