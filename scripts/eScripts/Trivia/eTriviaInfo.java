...
public class eTriviaInfo {
    ...
    public static void sendAnswer(String answer) {
        ...
        Thread thread = new Thread(() -> {
            try {
                ...
                eActions.updateStatus(eActions.getCurrentTimeFormatted() + " [eScripts.Trivia] Sleeping for " + sleepTime + "ms");
                Thread.sleep(sleepTime); // Randomized delay
                ctx.keyboard.sendKeys(writeAnswer.toString());
                triviaAnswer = null;
                eActions.updateStatus(eActions.getCurrentTimeFormatted() + " [eScripts.Trivia] Question answered!");
                eActions.updateStatus(eActions.getCurrentTimeFormatted() + " [eScripts.Trivia] A: " + answer);
    ...
    public enum TriviaQuestion {
        ...
        CRYSTAL_EQUIPMENT("rScripts.agility level is required to use crystal equipment", "50"),
