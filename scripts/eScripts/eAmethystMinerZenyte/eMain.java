...
public class eMain extends TaskScript implements LoopingScript {
    ...
    private void initializeMethods() {
        eScripts.BotUtils.eBanking bankingUtils = new eBanking(ctx);
        eScripts.BotUtils.eActions actionUtils = new eActions(ctx);
        eScripts.BotUtils.eData dataUtils = new eData(ctx);
        eScripts.Trivia.eTriviaInfo triviaInfo = new eTriviaInfo(ctx);
    }
    ...
    public void onExecute() {
        ...
        eScripts.BotUtils.eActions.specialAttackTool = true;
    }
    ...
    public void onProcess() {
        ...
        if (!botStarted) {
            eScripts.BotUtils.eActions.status = "Please start the bot!";
            return;
        }
        ...
        if (!eScripts.BotUtils.eActions.inArea(MINING_AREA)) {
            eScripts.BotUtils.eActions.status = "Searching for mining area...";
            takingStepsRMining();
            return;
        }

        if (ctx.inventory.inventoryFull()) {
            eScripts.BotUtils.eBanking.bankTask(true, 8, 1, false, -1, -1, inventoryPickaxes);
        } else {
            if (!localPlayer.isAnimating() && (System.currentTimeMillis() > (lastAnimation + eScripts.BotUtils.eActions.getRandomInt(1200, 12000)))) {
                miningTask();
        ...
        if (localPlayer.isAnimating()) {
            if (eScripts.BotUtils.eActions.specialAttackTool) {
                eScripts.BotUtils.eActions.specialAttack(ItemID.DRAGON_PICKAXE);
            }
        }

        int[] combinedItems = IntStream.concat(Arrays.stream(eActions.randomEventItems), IntStream.of(21347))
                .toArray();
        eScripts.BotUtils.eActions.handleGroundItem("Take", combinedItems);
    }
    ...
    private void miningTask() {
        ...
        if (BANK_AREA.containsPoint(ctx.players.getLocal().getLocation()) && !ctx.pathing.inMotion()) {
            eScripts.BotUtils.eActions.updateStatus("Heading to mining area");
            takingStepsRMining();
        }

        String objectName = "Crystals";
        SimpleObjects nearestSpot = (SimpleObjects) ctx.objects.populate().filter(objectName);
        eScripts.BotUtils.eActions.updateStatus("Looking for nearest mining spot...");
        ...
        while (!nearestSpot.isEmpty()) {
            ...
            if (nearestSpot.size() >= 2 && isOtherPlayerMining) {
                eScripts.BotUtils.eActions.updateStatus("Spot take... Looking for another one...");
                ...
                if (nearestObject.validateInteractable()) {
                    ...
                    if (objectReachable) {
                        ...
                        ctx.log(objectName + " found " + distance + " " + eScripts.BotUtils.eActions.pluralize(distance, "tile", "tiles") + " away");
                        eScripts.BotUtils.eActions.interactWith(nearestObject, actionName);
                        eScripts.BotUtils.eActions.updateStatus("Mining " + objectName.toLowerCase());
                        ctx.onCondition(localPlayer::isAnimating, 250, 10);
                        return;
                    } else {
                        eScripts.BotUtils.eActions.updateStatus("Next " + objectName.toLowerCase() + " spot is not reachable.");
                        nearestSpot = (SimpleObjects) nearestSpot.filter(otherTree -> !otherTree.equals(nearestObject));
                    }
                } else {
                    eScripts.BotUtils.eActions.updateStatus("No " + objectName.toLowerCase() + " found in the vicinity.");
                    return;
                }
            }
        }
        eScripts.BotUtils.eActions.updateStatus("No suitable " + objectName.toLowerCase() + " spot found nearby.");
    }
    ...
    public void onTerminate() {
        ...
        gptStarted = false;
        messageSaved = null;
        eScripts.BotUtils.eActions.specialAttackTool = false;
    }
    ...
    public void onChatMessage(ChatMessage m) {
        ...
        eScripts.eApiAccess.eAutoResponser.handleGptMessages(getType, senderName, formattedMessage);
        eScripts.Trivia.eTriviaInfo.handleBroadcastMessage(getType, gameMessage);
    }
